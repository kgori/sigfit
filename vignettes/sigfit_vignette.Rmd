...
As in the case of signature fitting (Example 1 above), the extracted signatures and exposures can be retrieved using the ```retrieve_pars``` function with ```par = "signatures"``` or ```par = "exposures"```. These signatures and exposures refer only to the estimated best number of signatures (in this case, four).
