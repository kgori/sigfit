...
#### Proof of concept: fitting signatures to simulated data
...
mcmc_samples <- sigfit::fit_signatures(counts = mutations, signatures = sigs[c(1, 5, 7, 11), ],
                                       iter = 1000, warmup = 500, chains = 1, seed = 1)
```
...
To retrieve the exposures from the MCMC samples, we can use the ```retrieve_pars``` function. This will
produce a list with three matrices, containing the mean value, and the values corresponding to the
lower and upper limits of the highest posterior density (HPD) interval (the Bayesian equivalent to a 
confidence interval), for each exposure in each sample. The ```prob``` argument can be used to indicate 
the desired probability for the HPD interval (by default, 95% HPD intervals are returned).
```{r retrieve_exposures}
exposures <- retrieve_pars(mcmc_samples, feature = "exposures", prob = 0.95)
names(exposures)
exposures$mean
```
...
```{r fig.width=18, fig.height=12, out.width = '100%', fig.align = "center", exposures}
sigfit::plot_exposures(mutations, mcmc_samples = mcmc_samples)
```
