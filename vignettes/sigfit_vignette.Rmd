...
author: "Kevin Gori and Adrian Baez-Ortega – September 2017"
...
```{r fig.width=17, fig.height=6, out.width="100%", plotsim, eval=FALSE}
par(mar = c(6,4,6,4))
...
```{r include=FALSE, silent_sampling, eval=FALSE}
...
```{r retrieve_exposures, eval=FALSE}
...
```{r fig.width=12, fig.height=5, out.width = '100%', fig.align = "center", eval=FALSE, exposures}
sigfit::plot_exposures(mutations, 
                       mcmc_samples = mcmc_samples, 
                       signature_names = rownames(sigs))
```

```{r fig.width=12, fig.height=5, out.width = '100%', fig.align = "center", echo=FALSE, silent_exposures, eval=FALSE}
par(mar=c(7,4,3,0))
sigfit::plot_exposures(mutations, 
                       mcmc_samples = mcmc_samples, 
                       signature_names = rownames(sigs))
...
```{r fig.width=25, fig.height=18, out.width = '100%', echo=FALSE, silent_reconstruct, eval=FALSE}
par(mar=c(6,6,6,2))
...
For this second example, we will use single-nucleotide variant data from a set of 21 breast cancer samples, published in [Nik-Zainal _et al._, _Cell_ 2012](http://dx.doi.org/10.1016/j.cell.2012.04.024). These data can be accessed using ```data("variants_21breast")```.

```{r load_mutations, eval=FALSE}
...
* Trinucleotide context of the variant (reference sequence between the positions immediately before and after the variant, e.g. "TCA"). This can be obtained from the reference genome that was used to call the variants, using a package like ```BSgenome```; however, sequence context information is frequently stored by the variant caller within the INFO field of the VCF file.
...
This set of 21 mutational catalogues can be loaded directly using ```data("counts_21breast")```.
...
```{r fig.width=24, fig.height=30, out.width = '100%', fig.align = "center", eval=FALSE, plot_spectra, eval=FALSE}
par(mfrow = c(7, 3))
plot_spectrum(counts_21breast)
```

```{r fig.width=24, fig.height=25, out.width = '100%', fig.align = "center", echo=FALSE, silent_plot_spectra, eval=FALSE}
par(mfrow = c(7, 3), mar = c(5,6,7,2))
plot_spectrum(counts_21breast)
```
...
- Fit signatures to 21 samples using "emu" (human-genome opportunities).
...
---
_Old vignette -- replace by stuff above_
...
```{r fig.width=24, fig.height=16, out.width = '100%', fig.align = "center", plotmultisim, eval=FALSE}
par(mfrow = c(2, 2), mar = c(6, 4, 6, 4))
for (i in 1:nrow(mmutations)) sigfit::plot_spectrum(spectra = mmutations[i, ])
```

```{r include=FALSE, silent_multisampling, eval=FALSE}
set.seed(1)
...
```{r fig.width=24, fig.height=16, out.width="100%", echo=FALSE, silent_multireconstruct, eval=FALSE}
par(mar = c(6, 4, 6, 4))
anon <- suppressWarnings(
