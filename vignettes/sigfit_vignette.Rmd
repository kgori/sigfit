...
set.seed(1)
probs <- as.matrix(sigs[, c(1, 5, 7, 11)]) %*% c(0.4, 0.3, 0.2, 0.1)
mutations <- t(rmultinom(10, 2000, probs))
colnames(mutations) <- rownames(sigs)
```

```{r plotmultisim}
#par(mfrow = c(5, 2))
for (i in 1:nrow(mutations)) barplot(mutations[i, ], cex.names = 0.4, las = 2)
```
...
And the exposures
```{r fig.width=7.5, fig.height=5, multiexposures}
sigfit::plot_exposures(mcmc_samples, prob = 0.9, thresh = 0.01,
                         cex.names = 0.7, las = 2)
```
