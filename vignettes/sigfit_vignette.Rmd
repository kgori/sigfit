...
```{r fig.width=17, fig.height=6, out.width="100%", plotsim}
par(mar = c(6,4,6,4))
...
```{r fig.width=12, fig.height=5, out.width = '100%', fig.align = "center", echo=FALSE, silent_exposures}
par(mar=c(7,4,3,0))
sigfit::plot_exposures(mutations, 
                       mcmc_samples = mcmc_samples, 
                       signature_names = rownames(sigs))
...
```{r fig.width=25, fig.height=18, out.width = '100%', echo=FALSE, silent_reconstruct}
par(mar=c(6,6,6,2))
...
For this second example, we will use single-nucleotide variant data from a set of 21 breast cancer samples, published in [Nik-Zainal _et al._, _Cell_ 2012](http://dx.doi.org/10.1016/j.cell.2012.04.024). These data can be accessed using ```data("mutations_21breast")```.
...
```{r fig.width=24, fig.height=30, out.width = '100%', fig.align = "center", eval=FALSE, plot_spectra}
par(mfrow = c(7, 3))
plot_spectrum(counts_21breast, 
              counts = TRUE)
```

```{r fig.width=24, fig.height=25, out.width = '100%', fig.align = "center", echo=FALSE, silent_plot_spectra}
par(mfrow = c(7, 3), mar = c(5,6,7,2))
plot_spectrum(counts_21breast, counts = TRUE)
```
