...
```{r reconstruct, fig.width=20, fig.height=17, out.width='100%', warning=FALSE, results="hide", echo=-1}
...
```{r convert_sigs_silent, echo=F, fig.width=20, fig.height=12, out.width="100%"}
...
One novelty in sigfit is the introduction of Fit-Ext models, which are able to extract mutational signatures while fitting a set of predefined signatures which are already known to be present in the samples. Such models are useful for the discovery of rare signatures for which there is some qualitative evidence, but insufficient support as to deconvolute them using conventional extraction methods; or in cases where only signature fitting is possible, yet the data feature a mutational pattern which cannot be captured by the available signatures.
