...
parameters {
    ...
    vector<lower=0.035>[C_nu] nu_raw;  // unscaled degrees of freedom (t model)
...
transformed parameters {
    ...
    vector[C_nu] nu;                   // scaled degrees of freedom (t model)
    if ((family == 3) && (robust == 1)) {
        nu = nu_raw * 30;
    }
...
model {
    ...
    if (family == 1) {
        ...
        // Normal model family
            ...
            // Normal model (MODIFIED: SIGMA = SQRT(MU))
            if (robust == 0) {
                for (g in 1:G) {
                    counts_real[g] ~ normal(expected_counts[g], sqrt(expected_counts[g]));
                }
            }
            
            // t model (MODIFIED: SIGMA = SQRT(MU))
            else {
                //nu ~ gamma(2, 0.1);
                nu_raw ~ normal(0, 1);
                for (g in 1:G) {
                    counts_real[g] ~ student_t(nu, expected_counts[g], sqrt(expected_counts[g]));
                }
            }
        }
    }
