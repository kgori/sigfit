functions {
    // TODO: fix this include - KG 29-04-19
...
data {
    ...
    matrix[G, C] counts_real;        // observed mutation counts (continuous case)
...
transformed data {
    ...
    int C_phi = ((family == 2) && (robust == 1)) ? 1 : 0;
    int C_nu = ((family == 3) && (robust == 1)) ? G : 0;
    int G_sigma = ((family == 3) && (robust == 0)) ? G : 0;
...
parameters {
    ...
    real<lower=1> nu[C_nu];          // degrees of freedom (t model)
    real<lower=0> phi[C_phi];          // overdispersion (neg bin model)
}
    ...
    if (family == 1) {
        ...
        if (family == 2) {
            ...
            // Negative binomial model
                ...
                for (g in 1:G) {
                    for (c in 1:C) {
                        counts_int[g, c] ~ neg_binomial_2(expected_counts[g, c], phi);
                    }
                }
            }
        }
        ...
        // Normal model family
            ...
            // t model
                ...
                for (g in 1:G) {
                    //for (c in 1:C) {
                    counts_real[g] ~ student_t(nu[g], expected_counts[g], 1);
                    //}
                }
            }
        }
