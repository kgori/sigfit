...
parameters {
    ...
    vector<lower=0>[C] phi;       // negative binomial overdispersion
}
...
model {
    ...
    phi ~ cauchy(0, 2.5);                 // overdispersion priors
    for (g in 1:G) {
        exposures[g] ~ dirichlet(kappa);  // exposure priors
        for (c in 1:C) {
            counts[g, c] ~ neg_binomial_2(expected_counts[g, c], phi[c]);  // neg binomial likelihood
        }
    }
}
